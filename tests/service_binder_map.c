...
void test_add2() {
	...
	shdrt_Intent intent = { .id = shdrt_ComponentIdentifier_make("shd", "intent"), .explicitId = true };
	shdrt_Intent intent2 = { .id = shdrt_ComponentIdentifier_make("shd", "intent2"), .explicitId = true };
	...
	TEST_ASSERT_TRUE(shdrt_ServiceIntentBinderMap_contains(&res->second, intent2));
}
void test_delete2() {
	shdrt_ServiceBinderMap map = { 0 };
...
void test_add_multiple() {
	...
	const shdrt_ServiceBinderMap_value* res = shdrt_ServiceBinderMap_get(&map, s);
	const shdrt_ServiceBinderMap_value* res2 = shdrt_ServiceBinderMap_get(&map, s);

	TEST_ASSERT_FALSE(res == NULL);
	TEST_ASSERT_FALSE(res2 == NULL);

	TEST_ASSERT_TRUE(shdrt_ServiceIntentBinderMap_contains(&res->second, intent));
	TEST_ASSERT_TRUE(shdrt_ServiceIntentBinderMap_contains(&res2->second, intent));
}
