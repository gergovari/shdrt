...
void setUp(void) {
  global = c_new(shdrt_ComponentIdentifier,
                 {.package = cstr_from(package), .name = cstr_from(name)});
  global2 = c_new(shdrt_ComponentIdentifier,
                  {.package = cstr_from(package2), .name = cstr_from(name2)});
}
...
void test_clone(void) {
  ...
  TEST_ASSERT_TRUE(
      strcmp(cstr_str(&global2->package), cstr_str(&id2.package)) != 0);
