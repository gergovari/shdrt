...
void test_add(void) {
  ...
  shdrt_ServiceContext *ctx =
      shdrt_ServiceContextMap_create(&map, *global_s, NULL, dummy_func);

  TEST_ASSERT_NOT_NULL(ctx);
  TEST_ASSERT_EQUAL_PTR(ctx->stop, dummy_func);

  const shdrt_ServiceContextMap_value *val =
      shdrt_ServiceContextMap_get(&map, *global_s);
...
void test_add_multiple(void) {
  shdrt_ServiceContextMap map = {0};
  shdrt_Service calc = {
      .id = {.package = cstr_from("shd"), .name = cstr_from("calculator")},
      .on_start_command = NULL,
      .on_create = dummy_on_create,
      .on_destroy = dummy_on_destroy,
      .on_bind = NULL};
  shdrt_Service comm = {
      .id = {.package = cstr_from("shd"), .name = cstr_from("communication")},
      .on_start_command = NULL,
      .on_create = dummy_on_create,
      .on_destroy = dummy_on_destroy,
      .on_bind = NULL};
  shdrt_Service sett = {
      .id = {.package = cstr_from("shd"), .name = cstr_from("settings")},
      .on_start_command = NULL,
      .on_create = dummy_on_create,
      .on_destroy = dummy_on_destroy,
      .on_bind = NULL};
  shdrt_Service thirdp = {
      .id = {.package = cstr_from("acme"), .name = cstr_from("banking")},
      .on_start_command = NULL,
      .on_create = dummy_on_create,
      .on_destroy = dummy_on_destroy,
      .on_bind = NULL};
  ...
  TEST_ASSERT_NOT_NULL(
      shdrt_ServiceContextMap_create(&map, thirdp, NULL, NULL));
...
void test_delete(void) {
  ...
  const shdrt_ServiceContextMap_value *val =
      shdrt_ServiceContextMap_get(&map, *global_s);
  const shdrt_ServiceContextMap_value *val2 =
      shdrt_ServiceContextMap_get(&map, *global_s2);
  TEST_ASSERT_TRUE(val);
