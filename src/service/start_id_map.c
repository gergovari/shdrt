...
bool shdrt_ServiceStartIdMap_stop(shdrt_ServiceStartIdMap* map, shdrt_ServiceStartId id) {
	...
	while (i.ref) {
		if (shdrt_Service_equals(&i.ref->second, &val->second)) return false;
		shdrt_ServiceStartIdMap_next(&i);
	}
...
void shdrt_ServiceStartIdMap_forget(shdrt_ServiceStartIdMap* map, shdrt_Service s) {
	...
	while (i.ref) if (shdrt_Service_equals(&i.ref->second, &s)) i = shdrt_ServiceStartIdMap_erase_at(map, i); else shdrt_ServiceStartIdMap_next(&i);
}
